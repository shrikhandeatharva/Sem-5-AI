...
It makes use of at most three addresses and one operator to represent an expression and the value computed at 
each instruction is stored in temporary variable generated by compiler. The compiler decides the 
order of operation given by three address code. 

Three Address Code is Used in Compiler Applications
Optimization, Code generation, Debugging, Language translation.
